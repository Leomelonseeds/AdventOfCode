...
public class Day24_2 {
    ...
    public static void main(String[] args) {
        ...
        bs = new ArrayList<>();
        Day24_1.MAX_X = input.get(0).length() - 1;
        Day24_1.MAX_Y = input.size() - 1;
        start = Arrays.asList(new Integer[] {1, 0});
        end = Arrays.asList(new Integer[] {Day24_1.MAX_X - 1, Day24_1.MAX_Y});
        ...
        init.add(start);
        Set<List<Integer>> init2 = new HashSet<>();
        init2.add(end);
        Set<List<Integer>> init3 = new HashSet<>();
        init3.add(start);
        int result = travel(1, init, false);
        result += travel(1, init2, true);
        result += travel(1, init3, false);
        Utils.println(result);
    }
    
    private static int travel(int minute, Set<List<Integer>> positions, boolean toStart) {
        // Move blizzards
        bs.forEach(b -> b.move());
        ...
        for (List<Integer> pos : new HashSet<>(positions)) {
            ...
            for (List<Integer> np : new HashSet<>(new_pos)) {
                if (np.equals(end) && !toStart || np.equals(start) && toStart) {
                    return minute;
                }
                int nx = np.get(0);
                int ny = np.get(1);
                if (!(np.equals(end) || np.equals(start)) && 
                   (nx <= 0 || nx >= Day24_1.MAX_X || 
                    ny <= 0 || ny >= Day24_1.MAX_Y ||
        ...
        return travel(minute + 1, positions, toStart);
    }
}
